...
void imu::remap_axis(Remap_config config) {
    using ax_rem = imu_registers::axis_remap;
    auto remap_config = static_cast<uint8_t>(config.z_axis);
    remap_config = remap_config << 2|| static_cast<uint8_t>(config.y_axis);
    remap_config = remap_config << 2 || static_cast<uint8_t>(config.x_axis);
}
