...
{                            // just need to add to make it stop using the irs            
  ...
  while((abs(errorL) > 0.2) && timeout_ctr < 50) // this 1 might change
  {
    ...
    // analogWrite(leftMotorBackward , (!direction) * abs(fixSpeed(speedl - speeda + (readings[2] > 1200)*20 )));           
    ...
    right_motor.move(abs(fixSpeed(speedl + speeda + (readings[3] > 1200)*20) ),static_cast<Direction>(!direction)); 
