...
bool maze[MAX_H][MAX_W][5] = {
    0}; // represents the maze, first 4 bits represent the walls N E S W, the
        // last bit represents the visiting status
...
std::array ir_array = {front_left_ir,
                       front_right_ir,
                       left_ir,
                       right_ir,
                       left_diagonal_ir,
...
void set_wall() {
    // ////Serial.println("\ncoloring walls" + String(curr_c - 1) + " " +
    // String(16 - curr_r)); for(int i=0;i<4;i++)
    // ////Serial.println(maze[curr_r][curr_c][i]);
...
bool isAccessible(char r, char c, int dir) {
    return !(maze[r][c][dir] ||
             maze[r + r_mov[dir]][c + c_mov[dir]][(dir + 2) % 4]);
}
...
void flood(bool goal = 1) { // make goal = 0 to change the goal to the start
    ...
    if (goal) {
        ...
        // enqueue(r_q, 14);
        // enqueue(c_q, 3);
        for (char x = MAX_W / 2 - 1; x < MAX_W / 2 + 1;
             x++) { // change middle cells with 0
...
bool moveTo(char r, char c) {
    // get where I want to move relative to abolute direction (y3ny lw el robot
    // bases north) ana lesa m2alef el term dah
    short dir;
    if (r < curr_r)
        dir = 0;
    if (r > curr_r)
    ...
    // compare the movement direction to the current directoin to know how
    // should I turn
    ...
    if (dir - curr_dir == -1 || dir - curr_dir == 3) // turn left
    {
        ...
        delay(100);
        // moveF(1);       //moveForward();
        curr_dir += 3; // b3mel +3 msh -1 because el negative numbers don't
                       // work/work differently fel mod//
        curr_dir %= 4;
    ...
    else if (dir - curr_dir == 1 || dir - curr_dir == -3) // turn right
    {
        ...
        Serial.println("moving forward");
        if (!moveF(1))
            return 0; // moveForward(); //this function returns 0 if it was
                      // unable to move,so we leave the func, 3shan man8ayarsh
                      // el curr c wel curr r
    }
...
void exploreToCenter() {
    motionSuccessful = 1;
    flooded = 0;
    while (!(((curr_r == MAX_H / 2 - 1) || (curr_r == MAX_H / 2)) &&
             ((curr_c == MAX_W / 2 - 1) || (curr_c == MAX_W / 2)))) {
...
void exploreToStart() {
    ...
    while (!(curr_c == 1 && curr_r == 16) && !menu) {
        Serial.println("Exploring to start" + String((int)curr_c) + " " +
                       String((int)curr_r) + String((int)curr_dir));
        // log("start: " + tostr(dis[curr_r][curr_c]));
        // log(String((int)curr_r)+" "+String((int)curr_c )+"
        // "+String((int)curr_dir));
...
// void setup() {
//   //Serial.begin(19200);
//   initialise(c_q,MAX_H * MAX_W); //queue initialisation for storing row and
//   coloumn initialise(r_q,MAX_H* MAX_W); update_mms_maze(); log("Khalast
//   setup");
...
bool frontEmergency() {
    READIRS();
    if (readings[0] > 1250 || readings[1] > 1250)
        return 1;
    return 0;
}
...
bool wallFront() {
    for (int i = 0; i < 10; i++) {
        READIRS();
        if (readings[0] > frontleftThresh && readings[1] > frontrightThresh)
            return 1;
    }
    return 0;
}
...
// 2 -1
//-1.2 1.5
bool moveF(double tiles = 16) // if you want to move tile by tile use moveF(1),
                              // if you want continuous use moveF();
{ // just need to add to make it stop using the irs
    double desiredDistance =
        tiles * 19.7; // el tile el mafrood 18cm, bas we found it would move
                      // slightly less than what we wanted, fa we increased it
    ...
    bool direction =
        (errorL >= 0 ? true : false); // true -> forward, false -> backward
    ...
    while ((abs(errorL) > 0.2) && timeout_ctr < 50) // this 1 might change
    {
        ...
        //     integralval += kiTicks*(deltaTicks - errorTicksPrev)*(millis() -
        //     t) ;
        // speedTicks = KpTicks*deltaTicks + KdTicks * (deltaTicks -
        // errorTicksPrev)/(millis() - t)  + integralval;
        ...
        analogWrite(
            ...
            direction *
                abs(fixSpeed(speedl -
                             speeda))); // might change the +- signs here
        analogWrite(
            leftMotorBackward,
            (!direction) *
                abs(fixSpeed(speedl -
                             speeda))); // also might add a constrain fa lw el
                                        // speed less/greater than the limits,
                                        // it doesn't overflow

        analogWrite(rightMotorForward,
                    direction * abs(fixSpeed(speedl + speeda)));
        analogWrite(rightMotorBackward,
                    (!direction) * abs(fixSpeed(speedl + speeda)));
...
inline float getRate() {
    ...
    return gyroData.gyro
        .x; // might change it to gyro.z msh x , haven't tested yet
            // -----------------------------------------------------------------------IMPORTANT
}
...
inline void getPosition() {
    ...
    double encoderDelta = (rightDistance - leftDistance) /
        distance_between_wheels; // won't use that , and this is in radian
...
void setup() {
    ...
    // Serial.begin(115200);
    initialise(
        c_q, MAX_H * MAX_W); // queue initialisation for storing row and coloumn
    ...
    // Set operation mode to NDOF_FMC_OFF (9-axis fusion with fast mag
    // calibration off)
