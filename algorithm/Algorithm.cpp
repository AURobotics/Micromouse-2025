...
void log(const std::string& text) {     /// copied that from the mms example 
...
void set_wall()
{
    ...
    std::cerr<<std::endl;
    if(maze[curr_r][curr_c][0])API::setWall(curr_c-1,16-curr_r,'n');
    if(maze[curr_r][curr_c][1])API::setWall(curr_c-1,16-curr_r,'e');
    if(maze[curr_r][curr_c][2])API::setWall(curr_c-1,16-curr_r,'s');
    if(maze[curr_r][curr_c][3])API::setWall(curr_c-1,16-curr_r,'w');
}
